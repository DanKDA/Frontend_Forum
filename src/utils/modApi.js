...
export const promoteModerator = async (communityId, targetUserId, token) => {
  const res = await apiFetch(`/api/Communities/${communityId}/moderators/${targetUserId}`, {
    method: 'POST',
    headers: authHeaders(),
  })
...
export const kickMember = async (communityId, targetUserId, token) => {
  const res = await apiFetch(`/api/Communities/${communityId}/members/${targetUserId}`, {
    method: 'DELETE',
    headers: authHeaders(),
  })
  const text = await safeText(res)
...
export const banMember = async (communityId, targetUserId, reason, token) => {
  const res = await apiFetch(`/api/Communities/${communityId}/ban/${targetUserId}`, {
    method: 'POST',
    headers: authHeaders(),
...
export const transferOwnership = async (communityId, newOwnerId, token) => {
  const res = await apiFetch(`/api/Communities/${communityId}/transfer/${newOwnerId}`, {
    method: 'POST',
    headers: authHeaders(),
  })
  const text = await safeText(res)
...
export const pinPost = async (communityId, postId, token) => {
  const res = await apiFetch(`/api/Communities/${communityId}/posts/${postId}/pin`, {
    method: 'POST',
    headers: authHeaders(),
  })
  const text = await safeText(res)
...
export const fetchCommunityPostsForMod = async (communityId, token) => {
  const res = await apiFetch(`/api/Posts/community/${communityId}?sortBy=new&page=1&pageSize=50`)
